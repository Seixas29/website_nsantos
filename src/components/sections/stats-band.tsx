...
export function StatsBand({ locale }: { locale: Locale }) {
  return (
    <section className="border-b border-[#d8e2ea] bg-white dark:border-[#1c2a38] dark:bg-[#0f1822]">
      <div className="mx-auto grid w-full max-w-7xl grid-cols-2 gap-8 px-6 py-12 md:grid-cols-4 md:gap-6 md:py-14">
        {stats.map((stat, i) => {
          ...
          return (
            ...
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-teal/10 text-teal dark:bg-teal/15">
                <Icon className="h-5 w-5" strokeWidth={1.75} />
              </span>
              <NumberTicker
                value={stat.value}
                suffix={stat.suffix}
                className="font-display text-4xl font-semibold text-[#142033] dark:text-[#e8eef5] md:text-5xl"
              />
